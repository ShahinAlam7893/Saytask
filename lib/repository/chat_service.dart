...
class ChatViewModel extends ChangeNotifier {
  ...
  Future<void> sendMessage(String message) async {
    ...
    try {
      ...
      if (metadata != null) {
        ...
        eventTitle = metadata['title'] as String? ?? aiMessage;
      ...
      // AUTO-SAVE: Save task/event/note to backend immediately
  ...
  Future<void> _saveToBackend(ChatMessage msg, Map<String, dynamic> apiResponse) async {
    if (isSaving) return;
    ...
    try {
      ...
      if (msg.responseType == 'task') {
        ...
        final endTimeStr = eventTime.add(const Duration(hours: 1)).toUtc().toIso8601String();
        final metadata = apiResponse['metadata'] as Map<String, dynamic>?;
    ...
    try {
      ...
      if (msg.type == MessageType.task) {
        final reminders = msg.callMe == true
            ? [
                {"time_before": 10, "types": ["notification", "call"]},
